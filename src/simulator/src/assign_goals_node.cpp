...
void selected_drones_cb(const geometry_msgs::PoseStamped::ConstPtr &msg) {
    ...
    cout.precision(3);
    cout << "received drone " << drone.drone_id << " at " << drone.p.transpose() << ", total:" << drones_.size()
         << endl;
...
void user_goal_cb(const geometry_msgs::PoseStamped::ConstPtr &msg) {
    ...
    for (size_t i = 0; i < drones_.size(); ++i) {
        ...
        cout.precision(3);
        cout << "drone " << drones_[i].drone_id << ", start=" << drones_[i].p.transpose() << ", end="
             << each_one_goals[i].transpose() << endl;
    }
...
int main(int argc, char **argv) {
    ...
    ros::Subscriber selected_drones_sub = nh.subscribe<geometry_msgs::PoseStamped>("/rviz_selected_drones", 100,
                                                                                   selected_drones_cb);
