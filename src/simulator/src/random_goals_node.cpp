...
void combined_odoms_sim_sub_cb(const nav_msgs::Odometry::ConstPtr &msg) {
    ...
    if (msg->child_frame_id.substr(0, 6) != string("drone_") || id >= (int) drones_.size()) {
        ROS_ERROR("[random_goals_node] Wrong child_frame_id: %s, or wrong drone_id: %d",
                  msg->child_frame_id.substr(0, 6).c_str(), id);
        return;
    }
...
int main(int argc, char **argv) {
    ...
    for (int i = 0; i < drone_num; ++i) {
        odoms_sim_sub[i] = nh.subscribe<nav_msgs::Odometry>("/drone_" + to_string(i) + "_visual_slam/odom", 1000,
                                                            boost::bind(odoms_sim_sub_cb, _1, i));
    }
        ...
        while (drone_trials < drone_num) {
            ...
            if (drones_[d_id].odom_received && drones_[d_id].arrived_for_a_while) {
                ...
                while (goal_trials < goal_num) {
                    ...
                    double ang = acos(((goals[g_id].p - drones_[d_id].cur_p).normalized()).dot(
                            (Eigen::Vector3d(cos(drones_[d_id].cur_yaw), sin(drones_[d_id].cur_yaw), 0)).normalized()));
