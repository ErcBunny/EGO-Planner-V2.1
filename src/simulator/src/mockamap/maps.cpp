...
#include "mockamap/perlinnoise.hpp"
...
Maps::recursiveDivision(int xl, int xh, int yl, int yh, Eigen::MatrixXi &maze) {
    ...
    if (xl < xh - 3 && yl < yh - 3) { // the remaining area is larger than or equal to 5*5, need to add both x
        // wall and y wall
        ...
        while (valid == false) {
            ...
            // add a wall at the sides)
            if (xl - 1 >= 0) { // there is a point at xl-1,ym
                if (maze(xl - 1, ym) == 0) { // this is an opening,need to change random number
                    continue;
                }
            } else if (xh + 1 <= maze.cols() - 1) { // there is a point at xh+1,ym
                if (maze(xh + 1, ym) == 0) { // this is an opening,need to change random number
                    continue;
                }
            } else if (yl - 1 >= 0) { // there is a point at xm,yl-1
                if (maze(xm, yl - 1) == 0) { // this is an opening,need to change random number
                    continue;
                }
            } else if (yh + 1 <= maze.rows() - 1) { // there is a point at xm,yh+1
                if (maze(xm, yh + 1) == 0) { // this is an opening,need to change random number
                    continue;
                }
            }
        ...
        std::cout << maze << std::endl;
        return;
    } else if (xl < xh - 1 && yl < yh - 2) { // the case of 3*4+
        ...
        //
    else if (xl < xh - 2 && yl < yh - 1) { // the case of 4+*3
    ...
    else if (xl < xh - 1 && yl < yh - 1) { // the case of 3*3
        maze(xl + 1, yl + 1) = 1;
        return;
    } else {
        ROS_INFO("finished generating maze with width %d , height %d",
...
Maps::Maze3DGen() {
    ...
    for (int i = 0; i < info.sizeX; i++) {
        for (int j = 0; j < info.sizeY; j++) {
            for (int k = 0; k < info.sizeZ; k++) { // for every scaled coordinate points
                pcl::PointXYZ test;
                ...
                if (std::abs(mp.getDist2() - mp.getDist1()) <
                    1 / info.scale) { // the tested location is on one of the middle planes
                    if ((mp.getPoint1() + mp.getPoint2()) >
                        int((1 - connectivity) * numNodes) &&
                        (mp.getPoint1() + mp.getPoint2()) <
                        int((1 + connectivity) * numNodes)) { // this is a holed wall
                        double judge =
