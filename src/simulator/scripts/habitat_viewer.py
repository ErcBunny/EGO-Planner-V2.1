# Adapted from habitat-sim/examples/viewer.py
# ProggyClean.tff is also from the habitat-sim
...
class HabitatSimInteractiveViewer(mn.platform.glfw.Application):
    ...
    def __init__(self, sim_settings: Dict[str, Any], rgb_pub) -> None:
        ...
        mn.platform.glfw.Application.__init__(self, configuration)
        self.fps: float = 60.0
        ...
        key = mn.platform.glfw.Application.KeyEvent.Key
        self.pressed = {
            key.UP: False,
            key.DOWN: False,
            key.LEFT: False,
        ...
        relative_path_to_font = "../assets/habitat/ProggyClean.ttf"
    ...
    def draw_event(
        ...
        if rospy.is_shutdown():
            self.exit_event(mn.platform.glfw.Application.ExitEvent)
            return
        ...
        if self.enable_batch_renderer:
            ...
            self.ros_publish(agent, self.sim.get_sensor_observations())
    ...
    def ros_publish(self, agent, obs) -> None:
    ...
    def key_press_event(self, event: mn.platform.glfw.Application.KeyEvent) -> None:
        """
        Handles `mn.platform.glfw.Application.KeyEvent` on a key press by performing the corresponding functions.
        ...
        """
        key = event.key
        pressed = mn.platform.glfw.Application.KeyEvent.Key
        mod = mn.platform.glfw.Application.InputEvent.Modifier
    ...
    def key_release_event(self, event: mn.platform.glfw.Application.KeyEvent) -> None:
        """
        Handles `mn.platform.glfw.Application.KeyEvent` on a key release. When a key is released, if it
    ...
    def mouse_move_event(
            self, event: mn.platform.glfw.Application.MouseMoveEvent
    ) -> None:
        """
        Handles `mn.platform.glfw.Application.MouseMoveEvent`. When in LOOK mode, enables the left
        ...
        """
        button = mn.platform.glfw.Application.MouseMoveEvent.Buttons
    ...
    def mouse_press_event(self, event: mn.platform.glfw.Application.MouseEvent) -> None:
        """
        Handles `mn.platform.glfw.Application.MouseEvent`. When in GRAB mode, click on
        objects to drag their position. (right-click for fixed constraints)
        """
        button = mn.platform.glfw.Application.MouseEvent.Button
    ...
    def mouse_scroll_event(
            self, event: mn.platform.glfw.Application.MouseScrollEvent
    ) -> None:
        """
        Handles `mn.platform.glfw.Application.MouseScrollEvent`. When in LOOK mode, enables camera
        ...
        shift_pressed = bool(
            event.modifiers & mn.platform.glfw.Application.InputEvent.Modifier.SHIFT
        )
        alt_pressed = bool(
            event.modifiers & mn.platform.glfw.Application.InputEvent.Modifier.ALT
        )
        ctrl_pressed = bool(
            event.modifiers & mn.platform.glfw.Application.InputEvent.Modifier.CTRL
        )
    ...
    def mouse_release_event(
            self, event: mn.platform.glfw.Application.MouseEvent
    ) -> None:
        """
    ...
    def exit_event(self, event: mn.platform.glfw.Application.ExitEvent):
        """
        Overrides exit_event to properly close the Simulator before exiting the
        mn.platform.glfw.application.
        """
        for i in range(self.num_env):
...
Key Commands:
-------------
    esc:        Exit the mn.platform.glfw.application.
...
if __name__ == "__main__":
    rospy.init_node("habitat_viewer")

    sim_settings: Dict[str, Any] = default_sim_settings
    sim_settings["scene"] = rospy.get_param("~scene")
    sim_settings["scene_dataset_config_file"] = rospy.get_param("~dataset")
    sim_settings["enable_physics"] = not rospy.get_param("~disable_physics")
    sim_settings["use_default_lighting"] = rospy.get_param("~use_default_lighting")
    sim_settings["enable_batch_renderer"] = rospy.get_param("~enable_batch_renderer")
    sim_settings["num_environments"] = rospy.get_param("~num_environments")
    sim_settings["composite_files"] = rospy.get_param("~composite_files")
    sim_settings["window_width"] = rospy.get_param("~width")
    sim_settings["window_height"] = rospy.get_param("~height")
    sim_settings["default_agent_navmesh"] = False
    sim_settings["enable_hbao"] = rospy.get_param("~hbao")
    sim_settings["color_sensor"] = True

    # start the application
    HabitatSimInteractiveViewer(
        sim_settings, rospy.Publisher("~rgb", Image, queue_size=1)
    ).exec()
