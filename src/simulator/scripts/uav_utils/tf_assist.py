...
class OdometryConverter(object):
    ...
    def __init__(self, frame_id_in_, frame_id_out_, broadcast_tf_, body_frame_id_, intermediate_frame_id_,
                 world_frame_id_):
        ...
        if self.broadcast_tf:
            rospy.loginfo('ROSTopic: [%s]->[%s] TF: [%s]-[%s]-[%s]',
                          self.frame_id_in, self.frame_id_out, self.body_frame_id, self.intermediate_frame_id,
                          self.world_frame_id)
        else:
