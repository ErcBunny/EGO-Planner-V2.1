...
poly_traj::Trajectory predict_traj(const double acc, const double dir, const Eigen::Vector3d p, const Eigen::Vector3d v,
    ...
    tailState << Eigen::Vector3d(tail_pv.first(0), tail_pv.first(1), p(2)), Eigen::Vector3d(tail_pv.second(0),
                                                                                            tail_pv.second(1),
                                                                                            v(2)), Eigen::Vector3d::Zero();
...
int main(int argc, char **argv) {
    ...
    predicted_traj_pub_ = nh.advertise<trajectory_msgs::MINCOTraj>("/broadcast_traj_to_planner", 10);
