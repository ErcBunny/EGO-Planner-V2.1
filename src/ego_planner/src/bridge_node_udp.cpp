...
#include <trajectory_msgs/MINCOTraj.h>
...
trajectory_msgs::MINCOTraj MINCOTraj_msg_;
...
void one_traj_sub_udp_cb(const trajectory_msgs::MINCOTrajPtr &msg) {
...
void udp_recv_fun() {
    ...
    while (true) {
        if ((valread = recvfrom(udp_server_fd_, udp_recv_buf_, BUF_LEN, 0, (struct sockaddr *) &addr_client,
...
int main(int argc, char **argv) {
    ...
    one_traj_sub_ = nh.subscribe("/broadcast_traj_from_planner", 100, one_traj_sub_udp_cb,
                                 ros::TransportHints().tcpNoDelay());
    one_traj_pub_ = nh.advertise<trajectory_msgs::MINCOTraj>("/broadcast_traj_to_planner", 100);
